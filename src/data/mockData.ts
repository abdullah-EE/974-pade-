...
export const players: Player[] = names.map((n,i)=>({id:`p${i+1}`,name:`${n} Al-${i+2}`,username:`@${n.toLowerCase()}${i+1}`,avatar:mockImages.players[i%mockImages.players.length],rank:i+1,rating:1690-i*8,wins:22-i%7,losses:6+i%6,movement:(i%7)-3,level:i<8?'Advanced':i<17?'Intermediate':'Beginner',hand:i%5===0?'Left':'Right',side:i%2?'Backhand':'Forehand',verified:i<20,hotStreak:i%6===0}));
...
].map((name,i)=>({id:`c${i+1}`,name,area:(['Lusail','Doha','The Pearl','West Bay','Aspire'] as const)[i%5],indoor:i%2===0,courts:4+(i%5),price:`QAR ${140+i*10}-${200+i*12}`,image:mockImages.courts[i%mockImages.courts.length],ranked:i%3!==0}));
